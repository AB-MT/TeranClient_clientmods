...
minetest.register_chatcommand("schemebuild", {
	...
	func = function(name)
		...
		if not rawdata then
			minetest.display_chat_message("Schematica '" .. name .. "' not found.")
			return false
		end
	...
	if building and not autoeat.eating then
		...
		if not new_index then
			if not out_of_blocks then
				...
				minetest.display_chat_message("[Schematicas] Missing ressource: " .. node)
			end
			out_of_blocks = true
			return
		end
		if out_of_blocks then
			minetest.display_chat_message("[Schematicas] Resuming.")
		end
		out_of_blocks = false
